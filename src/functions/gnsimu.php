...
class GNSimu
{
    ...
    $this->name[0] = "Jäger";
    $this->attackpower[0]  = array(0.0246, 0.392, 0.0263); // Wie viele Schiffe ein Schiff mit 100% Feuerkrafft zerstören würde
    $this->shiptoattack[0] = array(11,1,4); // Welche Schiffe/Geschütze angegriffen werden
    ...
    $this->mcost[9] = 6000;
    $this->kcost[9] = 2000;
    $this->name[9]    = "Leichtes Orbitalgeschütz";
            ...
            for($j=0;$j<count($this->attackpower[$i]);$j++)
        {
                ...
                if($Restpoweratt>0)
                {
                    ...
                    if($debug)
                    {
                      ...
                      echo "<font color=#ff0000>-</font> Maximale Zerstörung: floor(($RestPercentatt+".$this->percent[$i][$j].") * $OldRestpoweratt * ".$this->attackpower[$i][$j].")=$MaxDestruction<br>";
                    }
            ...
            $this->getlostkristall[1] += ($del*$this->kcost[$this->shiptoattack[$i][$j]]);
                ...
                if($Restpowerdeff>0)
                {
                    ...
                    if($debug)
                    {
                      ...
                      echo "<font color=#00ff00>-</font> Maximale Zerstörung: floor(($RestPercentdeff+".$this->percent[$i][$j].") * $OldRestpowerdeff * ".$this->attackpower[$i][$j].")=$MaxDestruction<br>";
                    }
    ...
    if (!$pretick) $pretick=0;
      ...
      for($i=0;$i<14;$i++)
      {
        ...
        //Schleife über Geschütze mit Preticks
      for($i=10;$i<13;$i++)
      {
            unset($templost);
