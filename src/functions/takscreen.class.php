    ...
    if(preg_match_all("/(\d{1,4}):(\d{1,2})(.*?)<br>/ism",$line[8],$tak_atterlist,PREG_SET_ORDER)) {
      if(count($tak_atterlist)){
        ...
        for($j=0;$j < count($tak_atterlist);$j++){
          ...
          if($tak_eta == "error") {$errors[] = "atter (".$tak_atterlist[$j][1].":".$tak_atterlist[$j][2].") zeit nicht erkannt";continue;}
          #5 ticks im orbit
          if ($tak_eta == 0) {
            $tak_eta_low = -75;
            $tak_eta_high = 0;
      ...
      if(preg_match_all("/(\d{1,4}):(\d{1,2})(.*?)<br>/ism",$line[10],$tak_list,PREG_SET_ORDER)) {
        if(count($tak_list)){
          ...
          for($j=0;$j < count($tak_list);$j++){
            unset($deffer);
            $tak_eta = $tak_timelist[$j];
            if($tak_eta == "error") {$errors[] = "deffer (".$tak_list[$j][1].":".$tak_list[$j][2].") zeit nicht erkannt";continue;}
            #20 ticks im orbit
            if ($tak_eta == 0) {
              $tak_eta_low = -300;
              $tak_eta_high = 0;
...
*/  #############################################################################
  function Atter_recallall() {
    if(!($id = param_num("id")) || !($user = getUserByID($id)) || !($atter = fleetstatus_get_bytarget($user['gala'],$user['pos'],1))) $this->_header();;
    if ($_REQUEST['send']) {
      if ($_REQUEST['yes_x']) {
